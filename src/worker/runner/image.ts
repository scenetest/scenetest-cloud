...
// Imported as base64 (generated by scripts/encode-assets.mjs): raw shell
// content would trip Cloudflare's WAF on worker upload, so we encode at
// build time and decode at runtime. Editing the source files rebuilds these.
import builderSetupB64 from './image/builder-setup-b64.ts'
import agentB64 from './image/agent-b64.ts'
const builderSetupTemplate = atob(builderSetupB64)
const agentSource = atob(agentB64)
...
export async function imageStageHash(): Promise<string> {
  ...
  cachedHash = (await sha256Hex(input)).slice(0, 16)
  return cachedHash
}
